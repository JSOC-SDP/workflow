...
while ($i_manager <= $n_managers)
    ...
    # there should be a manager.pid for this ticket in the active logs dir
        if (-e $WORKFLOW_DATA/tasks/$task/logs/$ticket/manager.pid) then
            set foundPID = `cat $WORKFLOW_DATA/tasks/$task/logs/$ticket/manager.pid`
            ...
            if (-e $WORKFLOW_DATA/tasks/$task/archive/logs/$ticket) then
                echo "       " this prog is for a completed task, ticket found $$WORKFLOW_DATA/tasks/$task/archive/logs/$ticket
