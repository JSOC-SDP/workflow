...
while ($keep_running > 0) # KEEPRUNNING LOOP
    sed -i 's/STATUS=[0-9]/STATUS=0/' $WORKFLOW_DATA/ticket.test
    echo " "
    echo " "
    echo " "
    ...
    foreach gate (`/bin/ls gates`) # GATE LOOP
        ...
        set gate_next_update = `cat nextupdate`
        set gate_update_delta = `cat updatedelta`
        set gate_type = `cat type`
        set gate_action_task = `cat actiontask`
        set gate_status = `cat gatestatus`
        set gate_status_task = `cat statustask`

        if ($gate_status == "HOLD") then
        ...
        foreach ticket (`/bin/ls new_tickets`) # TICKET LOOP (inside the gate loop)
            if ($verbosemode) echo "GATEKEEPER Start processing new ticket $ticket"
            echo $nowtxt > $WORKFLOW_DATA/LAST_NEWTICKET
	        ...
	        if ($ACTION == 1) then  # get low and high range for gate.
                ...
                set pending = `cat $WORKFLOW_DATA/tasks/$gate_action_task/state`
                @ pending = $pending + 1
                echo $pending >  $WORKFLOW_DATA/tasks/$gate_action_task/state
                set TASKMANAGER = `cat $WORKFLOW_DATA/tasks/$gate_action_task/manager`
                mkdir $WORKFLOW_DATA/tasks/$gate_action_task/logs/$ticket
                $WORKFLOW_DIR/$TASKMANAGER gate=$gate task=$gate_action_task ticket=$ticket >>&$WORKFLOW_DATA/tasks/$gate_action_task/logs/$ticket/manager.log  &
                echo $! > $WORKFLOW_DATA/tasks/$gate_action_task/logs/$ticket/manager.pid
        ...
        # gate_action_task is the task associated with this gate, the task that the ticket is sent to.
	    ...
	    set thisgatedir = $cwd
	    cd $WORKFLOW_DATA/tasks/$gate_action_task/done/
        if ($verbosemode) echo GATEKEEPER change to $cwd

        foreach donetask ( `/bin/ls` ) # DONE TASK LOOP
            # see if there is a ticket in this gate's gate_action_task done queue
            ...
            if ($lookroot == root) then
               echo found done task for $gate_action_task
               set root_done = 1
            else
               set root_done = 0
	        endif 

            foreach key (TICKET GATE TASKID) # KEY LOOP (inside done-task loop)
	        ...
	        ex - $donetask/ticket <<! ##### replacing this with a sed command causes parent tickets to fail
/STATUS/d
w
q
!
            # still in the done-task loop?
            ...
            if ($taskstate == 0) then
                ...
                echo " " >> $WORKFLOW_DATA/FAILED_TASKS
                echo -n "$gate_action_task FAILED for " >>$WORKFLOW_DATA/FAILED_TASKS
                echo -n `grep WANTLOW $WORKFLOW_DATA/tasks/$gate_action_task/archive/failed/$donetask/ticket` >> $WORKFLOW_DATA/FAILED_TASKS
                echo -n "  " >> $WORKFLOW_DATA/FAILED_TASKS
                echo -n   `grep WANTHIGH $WORKFLOW_DATA/tasks/$gate_action_task/archive/failed/$donetask/ticket` >> $WORKFLOW_DATA/FAILED_TASKS
                echo -n "  at " >> $WORKFLOW_DATA/FAILED_TASKS
                date >> $WORKFLOW_DATA/FAILED_TASKS
                echo "     See:" $WORKFLOW_DATA/tasks/$gate_action_task/archive/failed/$donetask >> $WORKFLOW_DATA/FAILED_TASKS
            endif
            ...
            if (-e $WORKFLOW_DATA/gates/$GATE/active_tickets/$TICKET) then
                mv $WORKFLOW_DATA/gates/$GATE/active_tickets/$TICKET $WORKFLOW_DATA/tasks/$parenttask/active/$TASKID/ticket_return
            endif

            if (-e $WORKFLOW_DATA/tasks/$parenttask/active/$TASKID/pending_tickets/$TICKET) then
                rm $WORKFLOW_DATA/tasks/$parenttask/active/$TASKID/pending_tickets/$TICKET
            else if (-e $WORKFLOW_DATA/tasks/$parenttask/archive/ok/$TASKID/pending_tickets/$TICKET) then
                # rm $WORKFLOW_DATA/tasks/$parenttask/active/$TASKID/pending_tickets/$TICKET
            else
                echo "GATEKEEPER done queue, FAILED to rm $WORKFLOW_DATA/tasks/$parenttask/active/$TASKID/pending_tickets/$TICKET"
            endif
        end # DONE TASK loop
        ...
        foreach ticket (`/bin/ls active_tickets`) # TICKET LOOP
            ...
            foreach key (ACTION STATUS TASKID EXPIRES WANTLOW WANTHIGH) # KEY LOOP (inside ticket loop)
...
!
                ...
                if (-e $WORKFLOW_DATA/tasks/$task/active/$TASKID) then
                    mv active_tickets/$ticket $WORKFLOW_DATA/tasks/$task/active/$TASKID/ticket_return
                    rm $WORKFLOW_DATA/tasks/$task/active/$TASKID/pending_tickets/$ticket
                else if (-e $WORKFLOW_DATA/tasks/$task/archive/ok/$TASKID) then
                    mv active_tickets/$ticket $WORKFLOW_DATA/tasks/$task/archive/ok/$TASKID/ticket_return
                    rm $WORKFLOW_DATA/tasks/$task/archive/ok/$TASKID/pending_tickets/$ticket
                else
                    echo "GATEKEEPER can not do mv active_tickets/$ticket $WORKFLOW_DATA/tasks/$task/active/$TASKID/ticket_return"
                endif
	        endif

            # still in active-ticket loop

            # if new slow ticket gate_type, initiate if new, else check status and wait
...
!
	        ...
	        if ($STATUS != 3 && $STATUS != 2) then   # 2,3 means still owned by some task, else return ticket
                ...
                # but check anyway
                if (-e $WORKFLOW_DATA/tasks/$task/active/$TASKID/ticket_return) then
                    if ($verbosemode) echo "GATEKEEPER active ticket exam, move active_tickets/$ticket to $task/active/$TASKID/ticket_return"
		            mv active_tickets/$ticket $WORKFLOW_DATA/tasks/$task/active/$TASKID/ticket_return
		            rm $WORKFLOW_DATA/tasks/$task/active/$TASKID/pending_tickets/$ticket
                endif

    		    mv active_tickets/$ticket $WORKFLOW_DATA/tickets_done
		    endif #$STATUS != 3 && $STATUS != 2
        end # active-TICKET LOOP
...
ALL_GATES_DONE:
    ...
    if (!(-e Keep_running)) then
        ...
        sed -i 's/STATUS=[0-9]/STATUS=5/' $WORKFLOW_DATA/ticket.test
